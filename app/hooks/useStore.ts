...
import {
  AppState, Tarefa, Cliente, AgendaRecorrente, AgendaExtra,
...
export function useStore() {
    ...
    async function init() {
      try {
        ...
        const { data: perfil } = await supabase
          .from('perfis').select('role').eq('id', uid_).single();
  ...
  const toggleGlobalView = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.message || 'Erro ao carregar visão global');
    } finally {
      setLoading(false);
    }
  }, [globalView, userRole]);
  ...
  const delCliente = useCallback(async (id: string) => {
    ...
    update(s => ({
      ...
      agendasExtras: s.agendasExtras.filter(e => e.clienteId !== id),
    }));
  }, [update]);
  ...
  const addTarefasBatch = useCallback(async (clienteId: string, descs: string[], prazo: string) => {
    ...
    const novas: Tarefa[] = descs.map(desc => ({
      id: uid(), clienteId, desc, prazo, status: 'pendente' as const,
      reuniaoId: last?.id ?? null, criadaEm: new Date().toISOString(), comentarios: [],
    }));
  ...
  const addComentario = useCallback(async (tarefaId: string, txt: string) => {
    ...
    update(s => ({
      ...
      tarefas: s.tarefas.map(t =>
        t.id === tarefaId ? { ...t, comentarios: [...t.comentarios, { txt, at }] } : t
      ),
    }));
  }, [update]);
  ...
  const delComentario = useCallback(async (tarefaId: string, idx: number) => {
    ...
    update(s => ({
      ...s,
      tarefas: s.tarefas.map(t =>
        t.id === tarefaId ? { ...t, comentarios: t.comentarios.filter((_, i) => i !== idx) } : t
      ),
    }));
  }, [update, state.tarefas]);
  ...
  const setAgendaExtraStatus = useCallback(async (id: string, status: string, motivo = '') => {
    ...
    update(s => ({
      ...s,
      agendasExtras: s.agendasExtras.map(e => e.id === id ? { ...e, status, motivo } : e),
    }));
  }, [update]);
  ...
  const setOcorrencia = useCallback(async (agendaId: string, date: string, status: 'ocorreu' | 'nao', motivo = '') => {
    ...
    update(s => ({
      ...s,
      ocorrencias: { ...s.ocorrencias, [key]: { status, motivo: status === 'ocorreu' ? '' : motivo } },
    }));
  }, [update]);
