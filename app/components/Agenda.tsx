...
import { getAgendaSlots, getScanSlots, ocorrenciaKey, scanKey } from '@/lib/agenda';
...
export default function Agenda({ store, showToast }: Props) {
  ...
  const extrasNoPeríodo = state.agendasExtras.filter(e => e.data >= fmtDate(hoje) && e.data <= fmtDate(em8)).sort((a, b) => a.data.localeCompare(b.data) || a.hora.localeCompare(b.hora));

  // Agrupa tudo por data
  const porData: Record<string, { recorrentes: typeof slots; extras: typeof extrasNoPeríodo; scans: typeof scanSlots }> = {};
  slots.forEach(s => { const k = fmtDate(s.date); if (!porData[k]) porData[k] = { recorrentes: [], extras: [], scans: [] }; porData[k].recorrentes.push(s); });
  extrasNoPeríodo.forEach(e => { if (!porData[e.data]) porData[e.data] = { recorrentes: [], extras: [], scans: [] }; porData[e.data].extras.push(e); });
  ...
  const tagRecorrente = <span style={{ padding: '2px 7px', borderRadius: 3, fontSize: 9, fontWeight: 800, letterSpacing: .5, textTransform: 'uppercase', background: 'var(--cyan-soft)', color: 'var(--cyan)', border: '1px solid rgba(13,219,255,.3)' }}>Recorrente</span>;
  ...
  // Dias disponíveis para scan (inclui sábado e domingo)
  const diasScan = [
  ...
  return (
    <div>
      ...
      {showForm && (
        <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 10, padding: 16, marginBottom: 14 }}>
          <div style={{ display: 'flex', gap: 0, marginBottom: 16, border: '1px solid var(--border)', borderRadius: 6, overflow: 'hidden' }}>
            {([['recorrente', '⟳ Recorrente', 'var(--cyan)'], ['extra', '+ Extra', 'var(--danger)'], ['scan', '🔍 Scan', 'var(--warn)']] as [FormTab, string, string][]).map(([tab, label, color]) => (
      ...
      {datasOrdenadas.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 30, color: 'var(--muted)', fontSize: 13 }}>Nenhuma agenda cadastrada. Clique em <strong>+ Adicionar</strong> para começar.</div>
      ) : (
                ...
                return (
                  <div key={key} style={{ display: 'flex', alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid var(--border)', gap: 12, background: bg }}>
                    ...
                    <div style={{ display: 'flex', gap: 5 }}>
                      ...
                      <button onClick={() => { if (confirm('Remover recorrência?')) { delAgenda(s.agendaId); showToast('Removida'); } }} style={{ padding: '4px 8px', borderRadius: 4, fontSize: 10, fontWeight: 800, cursor: 'pointer', border: '1px solid var(--border)', background: 'none', color: 'var(--muted)' }}>🗑</button>
                    </div>
                  </div>
                );
              })}
                ...
                return (
                  <div key={e.id} style={{ display: 'flex', alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid var(--border)', gap: 12, background: bg }}>
                    ...
                    <div style={{ display: 'flex', gap: 5 }}>
                      ...
                      <button onClick={() => { if (confirm('Remover agenda extra?')) { delAgendaExtra(e.id); showToast('Removida'); } }} style={{ padding: '4px 8px', borderRadius: 4, fontSize: 10, fontWeight: 800, cursor: 'pointer', border: '1px solid var(--border)', background: 'none', color: 'var(--muted)' }}>🗑</button>
                    </div>
                  </div>
                );
              })}
                ...
                return (
                  <div key={key} style={{ display: 'flex', alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid var(--border)', gap: 12, background: bg }}>
                    ...
                    <div style={{ display: 'flex', gap: 5 }}>
                      ...
                      <button onClick={() => { if (confirm('Remover scan?')) { delScan(s.scanId); showToast('Removido'); } }} style={{ padding: '4px 8px', borderRadius: 4, fontSize: 10, fontWeight: 800, cursor: 'pointer', border: '1px solid var(--border)', background: 'none', color: 'var(--muted)' }}>🗑</button>
                    </div>
                  </div>
                );
              })}
      ...
      {motivoKey && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(35,31,32,.75)', zIndex: 500, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(3px)' }} onClick={e => { if (e.target === e.currentTarget) setMotivoKey(null); }}>
          <div style={{ background: 'var(--ivory)', borderRadius: '16px 16px 0 0', padding: '22px 20px 28px', width: '100%', maxWidth: 660, borderTop: `2.5px solid ${motivoTipo === 'scan' ? 'var(--warn)' : 'var(--cyan)'}` }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <div style={{ fontWeight: 800, fontSize: 17 }}>Motivo <span style={{ color: motivoTipo === 'scan' ? 'var(--warn)' : 'var(--cyan-dim)' }}>Não Ocorreu</span></div>
            ...
            <button onClick={handleSaveMotivo} style={{ width: '100%', background: motivoTipo === 'scan' ? 'var(--warn)' : 'var(--cyan-dim)', color: motivoTipo === 'scan' ? 'white' : 'var(--dark)', border: 'none', borderRadius: 6, padding: 13, fontFamily: 'inherit', fontWeight: 800, fontSize: 13, cursor: 'pointer', textTransform: 'uppercase' }}>Salvar</button>
          </div>
        </div>
      )}
    </div>
