...
export default function Clientes({ store, showToast }: Props) {
  const { state, toggleConcluida, cycleStatus, delTarefa, addTarefa, addTarefasBatch, addComentario, delComentario } = store;
          ...
          <div key={c.id} style={{
            ...
            <div
              ...
              style={{
                ...
                borderLeft: `4px solid ${c.cor}`, background: 'white',
              }}
            >
              <div style={{
                width: 38, height: 38, borderRadius: 6, background: c.cor,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
                  {c.empresa || '—'} · {ts.length} tarefa{ts.length !== 1 ? 's' : ''} · {pct}%
                </div>
              ...
              <div style={{ borderTop: '1px solid var(--border)' }}>
                  ...
                  return (
                    <div key={t.id} style={{ background: 'var(--ivory2)', borderBottom: '1px solid var(--border)', opacity: t.status === 'concluida' ? .5 : 1 }}>
                      <div style={{ padding: '10px 16px 10px 20px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                        {/* Checkbox */}
                        <div
                          onClick={() => { toggleConcluida(t.id); showToast(t.status === 'concluida' ? 'Reaberta' : 'Concluída ✓'); }}
                          style={{
                            width: 18, height: 18, border: `2px solid ${t.status === 'concluida' ? 'var(--success)' : 'var(--border)'}`,
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 5, flexWrap: 'wrap' }}>
                            ...
                            <button className={`spill sp-${t.status}`} onClick={() => cycleStatus(t.id)}>{labelStatus(t.status)}</button>
                          </div>
                        </div>
                          ...
                          {t.comentarios.map((cm, i) => (
                            <div key={i} style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 5, padding: '8px 10px', marginBottom: 6 }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                                ...
                                <button onClick={() => delComentario(t.id, i)} style={{ fontSize: 11, color: 'var(--border)', cursor: 'pointer', background: 'none', border: 'none' }}>✕</button>
                              </div>
                ...
                {formOpen[c.id] && (
                  <div style={{ padding: '12px 16px', background: 'var(--ivory2)', borderTop: '1px solid var(--border)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                      <input
                        type="date"
                        value={formPrazo[c.id] || ''}
                        onChange={e => setFormPrazo(p => ({ ...p, [c.id]: e.target.value }))}
                        style={{ flex: 1, padding: '9px 12px', border: '1px solid var(--border)', borderRadius: 5, fontFamily: 'inherit', fontSize: 12, outline: 'none', background: 'white' }}
                      />
                      <select
                        value={formStatus[c.id] || 'pendente'}
                        onChange={e => setFormStatus(p => ({ ...p, [c.id]: e.target.value }))}
                        style={{ padding: '9px 12px', border: '1px solid var(--border)', borderRadius: 5, fontFamily: 'inherit', fontSize: 12, outline: 'none', background: 'white', cursor: 'pointer' }}
                      >
                ...
                {batchOpen === c.id && (
                  <div style={{ padding: '12px 16px', background: 'var(--ivory2)', borderTop: '1px solid var(--border)' }}>
                    ...
                    <textarea
                      rows={5}
                      placeholder="Cole os parágrafos aqui (parágrafo vazio = separador)..."
                      value={batchTxt}
                      onChange={e => setBatchTxt(e.target.value)}
                      style={{ width: '100%', padding: '9px 12px', border: '1px solid var(--border)', borderRadius: 5, fontFamily: 'inherit', fontSize: 12, outline: 'none', background: 'white', marginBottom: 8, resize: 'vertical', minHeight: 100 }}
                    />
