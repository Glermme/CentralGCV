...
export default function VizaoGeral({ store, showToast, onOpenReuniao }: Props) {
  ...
  // Clientes & Saúde
  const clientesComAbertas = clientes.filter(c =>
    tarefas.some(t => t.clienteId === c.id && t.status !== 'concluida' && t.status !== 'cancelada')
  ).length;
  const clientesComAtraso = clientes.filter(c =>
    tarefas.some(t => t.clienteId === c.id && isLate(t.prazo) && t.status !== 'concluida')
  ).length;

  // Próximas reuniões — 14 dias
  const hoje = new Date();
  const em14 = new Date(hoje); em14.setDate(hoje.getDate() + 14);
  const slots = getAgendaSlots(state.agendas, hoje, em14)
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        <button
          onClick={onOpenReuniao}
          style={{ background: 'transparent', border: '1.5px solid var(--cyan)', color: 'var(--cyan)', borderRadius: 5, padding: '5px 12px', fontFamily: 'inherit', fontWeight: 700, fontSize: 10, letterSpacing: 1, textTransform: 'uppercase', cursor: 'pointer' }}
        >+ Registrar Reunião</button>
      </div>
      ...
      {Object.keys(porData).length === 0 ? (
        ...
        Object.entries(porData).slice(0, 3).map(([dt, ss]) => {
          ...
          return (
            <div key={dt} style={{ background: 'var(--dark)', borderRadius: 10, overflow: 'hidden', marginBottom: 10, border: '1px solid var(--dark3)' }}>
      ...
      {/* Clientes & Saúde */}
      <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 18px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 8, background: 'var(--dark)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>👥</div>
          <div>
            <div style={{ fontWeight: 800, fontSize: 22, color: 'var(--dark)', lineHeight: 1 }}>{clientes.length}</div>
            <div style={{ fontSize: 10, color: 'var(--muted)', fontWeight: 600, letterSpacing: .8, textTransform: 'uppercase' }}>Clientes</div>
          </div>
        </div>
        <div style={{ width: 1, height: 32, background: 'var(--border)' }} />
        <div>
          <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--warn)' }}>{clientesComAbertas} com tarefas em aberto</div>
          {clientesComAtraso > 0 && (
            <div style={{ fontWeight: 700, fontSize: 12, color: 'var(--danger)', marginTop: 2 }}>⚠ {clientesComAtraso} com tarefas atrasadas</div>
          )}
          {clientesComAtraso === 0 && (
            <div style={{ fontWeight: 700, fontSize: 12, color: 'var(--success)', marginTop: 2 }}>✓ Nenhum cliente com atraso</div>
          )}
        </div>
      </div>
