...
import { isLate, fmtBR, fmtDate, labelStatus, DIAS_SEM } from '@/lib/store';
...
export default function VizaoGeral({ store, showToast, onOpenReuniao }: Props) {
  ...
  // Próximas reuniões — 14 dias, agrupadas por data
  const hoje = new Date();
  ...
  const reunIdxSafe = Math.min(reunIdx, datas.length - 1);

  // Próximos scans — 14 dias
  ...
  scanSlots.forEach(s => {
    const k = fmtDate(s.date);
    if (!porDataScan[k]) porDataScan[k] = [];
    porDataScan[k].push(s);
  });
  const datasScan = Object.keys(porDataScan).sort();
  const scanIdxSafe = Math.min(scanIdx, datasScan.length - 1);
  ...
  return (
    <div>
      ...
      {state.scans.length > 0 && (
        <>
          <div style={{ fontWeight: 800, fontSize: 10, letterSpacing: 2.5, textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ display: 'inline-block', width: 14, height: 2, background: 'var(--warn)', borderRadius: 1 }} />
            Próximos Scans
          </div>
                ...
                {porDataScan[datasScan[scanIdxSafe]]?.map(s => {
                  ...
                  return (
                    <div key={key} style={{ display: 'flex', alignItems: 'center', padding: '11px 16px', borderBottom: '1px solid var(--dark3)', gap: 12, background: oc.status === 'ocorreu' ? 'rgba(42,170,90,.05)' : oc.status === 'nao' ? 'rgba(232,48,48,.05)' : 'transparent' }}>
                      ...
                      <div style={{ display: 'flex', gap: 5 }}>
                        <button onClick={() => { setScanStatus(s.scanId, fmtDate(s.date), 'ocorreu'); showToast('Scan marcado como executado ✓'); }}
                        ...
                        <button onClick={() => { setMotivoScan({ scanId: s.scanId, data: fmtDate(s.date) }); setMotivoTxt(oc.motivo || ''); }}
      ...
      {/* ── CLIENTES & SAÚDE (estilo dark igual aos stats) ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
        ...
        <div style={{ background: 'var(--dark)', borderRadius: 8, padding: '14px 14px 12px', borderLeft: '3px solid var(--cyan)' }}>
          ...
          {clientesComAtraso > 0 && (
            <div style={{ fontSize: 10, color: 'var(--danger)', fontWeight: 700, marginTop: 2 }}>⚠ {clientesComAtraso} atrasados</div>
          )}
        </div>
